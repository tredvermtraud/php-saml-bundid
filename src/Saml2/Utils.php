...
class Utils
{
   ...
   *
   * @return string|null $x509 Formatted cert
   */
   ...
   * @param bool   $heads True if we want to include head and footer
   *
   * @return string|null $rsaKey Formatted private key
   */
   ...
   *
   * @return int|null $timestamp  Converted to a unix timestamp.
   *
   * @throws Exception
   */
   ...
   *
   * @return string|null Formatted fingerprint
   */
  ...
  public static function addSign($xml, $key, $cert, $signAlgorithm = XMLSecurityKey::RSA_SHA256, $digestAlgorithm = XMLSecurityDSig::SHA256)
  {
    ...
    if(in_array($rootNode->localName, $messageTypes)) {
      $issuerNodes = self::query($dom, '/' . $rootNode->nodeName . '/saml:Issuer');
